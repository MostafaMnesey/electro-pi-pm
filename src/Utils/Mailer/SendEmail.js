...
export const sendEmail = async ({
  ...
  if (!transporter) {
    console.warn("⚠️ Mailer Warning: transporter is not configured or nodemailer is not installed.");
    return { success: false, error: "Mailer not configured" };
  }

  const emailSubject = subject || "Notification";
  const emailText = text || `Your verification code is: ${otp || 'N/A'}`;
  const html = mailTemp({
    otp,
    title: emailSubject,
    text,
...
async function saveToImapSent(mailOptions) {
  ...
  try {
    ...
    console.warn("⚠️ IMAP save skipped because dependencies are missing or failed:", err.message);
  }
}
